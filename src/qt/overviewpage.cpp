...
void OverviewPage::updateDarksendProgress()
{
    ...
    QString strAnonymizeCadsAmount = BitcoinUnits::formatHtmlWithUnit(nDisplayUnit, nAnonymizeCadsAmount * COIN, false, BitcoinUnits::separatorAlways);
    ...
    if(currentBalance == 0)
    {
        ...
        strAnonymizeCadsAmount = strAnonymizeCadsAmount.remove(strAnonymizeCadsAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeCadsAmount + " / " + tr("%n Rounds", "", nDarksendRounds);
    ...
    if(nMaxToAnonymize > nAnonymizeCadsAmount*COIN) nMaxToAnonymize = nAnonymizeCadsAmount*COIN;

    if(nMaxToAnonymize == 0) return;

    if(nMaxToAnonymize >= nAnonymizeCadsAmount * COIN) {
        ui->labelAmountRounds->setToolTip(tr("Found enough compatible inputs to anonymize %1")
                                          .arg(strAnonymizeCadsAmount));
        strAnonymizeCadsAmount = strAnonymizeCadsAmount.remove(strAnonymizeCadsAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeCadsAmount + " / " + tr("%n Rounds", "", nDarksendRounds);
    } else {
