...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nAnonymizeCadsAmount"))
        settings.setValue("nAnonymizeCadsAmount", 1000);
    nAnonymizeCadsAmount = settings.value("nAnonymizeCadsAmount").toLongLong();
    if (settings.contains("nDarksendRounds"))
        SoftSetArg("-darksendrounds", settings.value("nDarksendRounds").toString().toStdString());
    if (settings.contains("nAnonymizeCadsAmount"))
        SoftSetArg("-anonymizetraveamount", settings.value("nAnonymizeCadsAmount").toString().toStdString());
...
QVariant OptionsModel::data(const QModelIndex & index, int role) const
{
    if(role == Qt::EditRole)
    {
        ...
        case AnonymizeCadsAmount:
            return QVariant(nAnonymizeCadsAmount);
        case UseBlackTheme:
...
bool OptionsModel::setData(const QModelIndex & index, const QVariant & value, int role)
{
    ...
    if(role == Qt::EditRole)
    {
        ...
        case DarksendRounds:
            ...
            break;
        case AnonymizeCadsAmount:
            nAnonymizeCadsAmount = value.toInt();
            settings.setValue("nAnonymizeCadsAmount", nAnonymizeCadsAmount);
            emit AnonymizeCadsAmountChanged(nAnonymizeCadsAmount);
            break;
        default:
            break;
        }
