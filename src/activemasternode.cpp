...
//
// Bootup the masternode, look for a 20000 CADS input and register on the network
//
void CActiveMasternode::ManageStatus()
{
    ...
    if(status == MASTERNODE_NOT_PROCESSED) {
        ...
        if(GetMasterNodeVin(vin, pubKeyCollateralAddress, keyCollateralAddress)) {
            ...
            /* rewards are not supported in cads.conf */
...
{
    CCryptoAdSolutionAddress address(collateralAddress);
