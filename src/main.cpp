...
const string strMessageMagic = "Cads Signed Message:\n";
...
int64_t GetProofOfWorkReward(int nHeight, int64_t nFees)
{
    ...
    if (nHeight == 1) 
	{
        nSubsidy = 20000000 * COIN; //20M Premine
    }
	else if (nHeight < 10)
	{
        nSubsidy = 10000 * COIN;
...
int64_t GetProofOfStakeReward(const CBlockIndex* pindexPrev, int64_t nCoinAge, int64_t nFees)
{
		...
		else if(pindexBest->nHeight < 220000)
    {
    	...
    	else if(pindexBest->nHeight < 260000)
    {
        nSubsidy = 15 * COIN;
    }
...
{
    int64_t ret = blockValue * 0.90; //90%
